...
class AiServiceClientTest {
    ...
    void calculateMatchScore_whenResponseMissingScore_returnsZero() {
        Map<String, Object> responseBody = Map.of("skillsMatched", List.of());
        when(restTemplate.exchange(anyString(), any(HttpMethod.class), any(), any(ParameterizedTypeReference.class)))
                .thenReturn(ResponseEntity.ok(responseBody));
    ...
    void recordInteraction_returnsTrue() {
        when(restTemplate.exchange(anyString(), any(HttpMethod.class), any(), any(ParameterizedTypeReference.class)))
                .thenReturn(ResponseEntity.ok(Map.of("status", "logged")));
    ...
    void checkDrift_returnsResult() {
        Map<String, Object> responseBody = Map.of("drift", false, "driftScore", 0.05);
        when(restTemplate.exchange(anyString(), any(HttpMethod.class), any(), any(ParameterizedTypeReference.class)))
                .thenReturn(ResponseEntity.ok(responseBody));

        Map<String, Object> result = aiServiceClient.checkDrift(List.of(85.0, 90.0), 0.1);
        assertFalse((Boolean) result.get("drift"));
    }
}